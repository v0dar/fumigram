...
if ($action == 'success' && IS_LOGGED && $config['paystack'] == "on" && !empty($config['paystack_secret_key']) && !empty($_GET['reference'])) {
    ...
    if ($payment) {
        ...
        $db->insert(T_TRANSACTIONS,array('user_id' => $me['user_id'],'amount' => $amount,'type' => 'Added money to wallet using Paystack','time' => time()));
        $notif   = new notify();
            if($update) {
                $re_data = array(
    ...
    if (!empty($_POST['amount']) && is_numeric($_POST['amount']) && $_POST['amount'] > 0) {
        ...
        $amount = $amount * 100;
        $callback_url = $config['site_url'] . "/aj/paystack/success&amount=".$main_total; 
        $result = array();
        $reference = uniqid();
