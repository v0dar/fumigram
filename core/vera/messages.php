...
class messages extends users{
	public function getMessages($to_id = false,$offset = false,$new = false,$order = 'ASC',$e = '>'){
		...
		$update  = array();
		$sql     = sql('chat/get.conversation.data',array(
		...
		if (is_array($messages) && count($messages) > 0) {
			foreach ($messages as $message) {
				...
				//$message->text = $this->linkifyText($message->text);
			}

			if (!empty($update)) {
				self::$db->where('id',$update,"IN");
				self::$db->update(T_MESSAGES,array('seen' => time()));
			}
			$data = $messages;
		}
		return $data;
	...
	public function messageData($msg_id = false){
		...
		if (!empty($data)) {
			$data->text = $this->linkifyText($data->text);
	...
	public function getChats($offset = false){
		...
		$data    = array();
		$user_id = $this->user_id;
		$sql     = sql('chat/get.chat.history',array(
			't_messages' => T_MESSAGES,
			't_chats' => T_CHATS,
			't_users' => T_USERS,
	...
	public function deleteEveryoneMessages($to_id = null,$messages = array()){
		if (empty($this->user_id) || empty($to_id)) {
			return false;
		}
		...
		self::$db->where('id',$messages,"IN");
		self::$db->where('from_id',$user_id);
		$q1 = self::$db->update(T_MESSAGES,array('deleted_fs1' => '1', 'deleted_fs2' => '1'));
		
		self::$db->where('id',$messages,"IN");
		self::$db->where('to_id',$user_id);
		$q2 = self::$db->update(T_MESSAGES,array('deleted_fs1' => '1', 'deleted_fs2' => '1'));
