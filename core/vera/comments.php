...
/* The main comments class which contains all the data about post comments */
class comments extends posts{
	public function insertCommentLike($info = array()){
		...
		$like_id    = 0;
		$comment_id = aura::secure($info['comment_id']);
		$user_id    = self::$me->user_id;
		if (!empty($info['user_id'])) {
			$user_id = aura::secure($info['user_id']);
		}
		$like_id = self::$db->insert(T_COMMENTS_LIKES,array('comment_id' => $comment_id, 'user_id'    => $user_id));
		return $like_id;
	}

	public function addCommentReply($info = array()){
		$info['comment_id'] = aura::secure($info['comment_id']);
		$info['user_id']    = self::$me->user_id;
		if (!empty($info['user_id'])) {
			$info['user_id'] = aura::secure($info['user_id']);
		}
		if (!empty($info['text'])) {
	...
	public function insertCommentReplyLike($info = array()){
		...
		$like_id    = 0;
		$reply_id = aura::secure($info['reply_id']);
		$user_id    = self::$me->user_id;
		if (!empty($info['user_id'])) {
			$user_id = aura::secure($info['user_id']);
		}
		$like_id = self::$db->insert(T_COMMENTS_REPLY_LIKES,array('reply_id' => $reply_id, 'user_id'    => $user_id));
		return $like_id;
	}
